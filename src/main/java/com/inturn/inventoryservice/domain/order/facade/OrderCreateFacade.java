...
public class OrderCreateFacade {
    ...
    public void createOrder(CreateOrderRecord order) {
        ...
        //주문 저장
        OrderEntity orderEntity = orderCommandService.save(OrderEntity.of(order));

        //주문 상세 저장
        order.itemList().forEach(o -> orderDetailCommandService.save(OrderDetailEntity.from(o, orderEntity.getOrderId())));

        publisher.publishEvent(new CreateOrderEvent(orderEntity.getOrderId(), order.userId(), order.address(), order.itemList()));
    }
